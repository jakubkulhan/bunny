...
final class Connection implements EventEmitterInterface
{
    ...
    private function onFrameReceived(AbstractFrame $frame): void
    {
        if ($frame instanceof MethodConnectionCloseFrame) {
            $this->client->disconnect(Constants::STATUS_CONNECTION_FORCED, sprintf('Connection closed by server: (%d) %s', $frame->replyCode, $frame->replyText), ClientInterface::RAW_CONNECTION_INACTIVE);
